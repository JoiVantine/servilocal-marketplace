...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    // Notifica cliente por e-mail quando prestador envia mensagem (fire-and-forget)
        ...
        try {
          ...
          if (clientUser?.email) {
            await sendMail(clientUser.email, 'cliente_nova_mensagem', {
              ...
              PREVIEW_MENSAGEM: (messageText || '').substring(0, 120) || 'Mensagem recebida',
              LINK_CHAT: `${baseUrl}/chat/${conversation.id}`,
            }, {
          ...
          console.error('[mail] cliente_nova_mensagem:', err.message);
        }
      });
    }
